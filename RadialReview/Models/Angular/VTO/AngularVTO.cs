...
#pragma warning restore CS0618 // Type or member is obsolete
		...
		public void ReplaceVision(AngularVTO vto) {
			// Id = vto.Id;
			//L10Recurrence = vto.L10Recurrence,
			//CreateTime = vto.CreateTime,
			//CopiedFrom = vto.CopiedFrom,
			...
			Values = vto.Values;//AngularCompanyValue.Create(vto._Values);
			CoreFocus = vto.CoreFocus;//AngularCoreFocus.Create(vto.CoreFocus);
			Strategy = vto.Strategy;//AngularStrategy.Create(vto.MarketingStrategy); //vto._MarketingStrategyModel.Select(x => AngularStrategy.Create(x));
			Strategies = vto.Strategies;//vto._MarketingStrategyModel.Select(x => AngularStrategy.Create(x)).ToList();
