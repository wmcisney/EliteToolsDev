...
namespace RadialReview.Accessors.PDF {
	public partial class AccountabilityChartPDF {
		...
		public class ACNode : D3.Layout.node<ACNode> {
			...
			public NodeEmphasis Emphasis = NodeEmphasis.Normal;
		}
		...
		private static ACNode dive(AngularAccountabilityNode aanode, TreeSettings settings) {
			...
			if (aanode.Group != null) {
				...
				if (aanode.Group.RoleGroups != null)
					roles = aanode.Group.RoleGroups.SelectMany(x => x.Roles.OrderBy(y => y.Ordering).Select(y => y.Name)).ToList();
			}
			...
			var node = new ACNode() {
				...
				Roles = roles,
				Id = aanode.Id,
				children = children.Select(x => dive(x, settings)).OrderBy(x => x.order).ToList(),
				...
				Emphasis = GetEmphasis(aanode.Highlight)
			};
		...
		private static ACNode diveSelectedNode(AngularAccountabilityNode aanode, TreeSettings settings, List<long> selectedNodes) {
			...
			children = children.Where(t => selectedNodes.Contains(t.Id)).OrderBy(x => x.order);
			...
			var node = new ACNode() {
				...
				Roles = roles,
				Id = aanode.Id,
				children = children.Select(x => diveSelectedNode(x, settings, selectedNodes)).OrderBy(x => x.order).ToList(),
				width = settings.baseWidth - settings.hSeparation,
				height = settings.baseHeight,
				hasHiddenChildren = aanode.HasChildren() && aanode.collapsed,
				order = aanode.order ?? 0,
		...
		private static NodeEmphasis GetEmphasis(bool? highlight) {
			if (highlight == null)
				return NodeEmphasis.Normal;
			if (highlight.Value)
				return NodeEmphasis.Emphasize;
			if (!highlight.Value)
				return NodeEmphasis.Deemphasize;
			return NodeEmphasis.Normal;
		}
		...
		public static MultiPageDocument GenerateAccountabilityChartSmartLevelsMultiDocumentsPerPage(AngularAccountabilityChart tree, List<AngularAccountabilityNode> highlights, XUnit width, XUnit height, AccountabilityChartSettings pageProps, bool restrictSize = false, TreeSettings settings = null) {
			//ENTRY POINT
			var docsAndStats = GenerateAccountabilityChartSmartLevels(tree, highlights, width, height, pageProps, restrictSize, settings);
			var docSettings = new MultiPageDocument.Settings() {
				MaxScale = 1.0,
				OutputSize = new XSize(width, height)
			};
		...
		public static List<PdfDocumentAndStats> GenerateAccountabilityChartSmartLevels(AngularAccountabilityChart tree, List<AngularAccountabilityNode> highlights, XUnit width, XUnit height, AccountabilityChartSettings pageProps, bool restrictSize = false, TreeSettings settings = null) {
			var settingsEmpty = settings == null;
			var docs = new List<PdfDocumentAndStats>();
			highlights = highlights.Where(x => x != null).Distinct(x => x.Id).ToList();

			Func<AngularAccountabilityNode, bool, bool> diveFunc = null;
			diveFunc = new Func<AngularAccountabilityNode, bool, bool>((me, parentActive) => {
				var isHighlight = highlights.Any(x => x.Id == me.Id);
				var anyHighlights = isHighlight;
				var toRemove = new List<AngularAccountabilityNode>();
				foreach (var c in me.children) {
					//Unhighlighted nodes with active children are connectors
					var anyActiveChildren = diveFunc(c, isHighlight);
					if (!isHighlight && !anyActiveChildren && !parentActive) {
						//Neither highlight, nor a connector
						toRemove.Add(c);
					}
					anyHighlights |= anyActiveChildren;
				}

				//Remove nodes that are neither connectors, nor highlighted.
				me.children = me.children.Where(x => !toRemove.Any(y => y.Id == x.Id)).ToList();
				...
				me.Highlight = isHighlight;

				return anyHighlights;
			});

			diveFunc(tree.Root, false);

			var useSettings = settings ?? new TreeSettings() {
				compact = true
			};

			return tree.Root.children.Where(x => x.collapsed == false || highlights.Any(y => y.Id == x.Id))
								.Select(x => GenerateAccountabilityChart(x, width, height, pageProps, restrictSize, useSettings, false))
								.ToList();
		}
		...
		public static PdfDocumentAndStats GenerateAccountabilityChart(AngularAccountabilityNode root, XUnit width, XUnit height, AccountabilityChartSettings pageProps, bool restrictSize = false, TreeSettings settings = null, bool anyAboveRoot = false, List<long> selectedNode = null) {
			...
			pageProps2.demphasisColor = XColors.LightGray;
		...
		private static XRect ACDrawRole(XGraphics gfx, ACNode me, AccountabilityChartSettings pageProps, double[] origin = null) {
			...
			gfx.DrawRectangle(pageProps.boxPen(me.Emphasis), pageProps.brush, x, y, width, height);
		...
		private static XRect ACDrawRoleLine(XGraphics gfx, ACNode parent, ACNode me, AccountabilityChartSettings pageProps, TreeSettings settings, double[] origin = null) {
			...
			if (me.isLeaf && parent.side != "left" && parent.side != "right") {
				...
				var minorAdjustment = 0;//.5;
				var tyy = ty - 1;
				if (!parent.isLeaf) {
					points.Add(Tuple.Create(sx, sy));
					points.Add(Tuple.Create(sx, my));
					points.Add(Tuple.Create(lx + minorAdjustment, my));
					//tyy = tyy + 1;
				} else {
					var ay = (sy - parent.height) + Math.Min(10, parent.height / 2) - adjS;
					points.Add(Tuple.Create(lx + minorAdjustment, ay));
				}


				points.Add(Tuple.Create(lx + minorAdjustment, tyy + minorAdjustment));
				points.Add(Tuple.Create(tx + minorAdjustment, tyy + minorAdjustment));
		...
		private static XRect ACDrawRoles(XGraphics gfx, ACNode me, AccountabilityChartSettings pageProps, TreeSettings settings, double[] origin = null, bool anyAboveRoot = false) {
			XRect boundary = XRect.Empty;

			if (me.children != null) {
				foreach (var c in me.children) {
					boundary.Union(ACDrawRoleLine(gfx, me, c, pageProps, settings, origin));
					boundary.Union(ACDrawRoles(gfx, c, pageProps, settings, origin));
				}
			}
			boundary.Union(ACDrawRole(gfx, me, pageProps, origin));
			if (me.hasHiddenChildren) {
				boundary.Union(ACDrawEllipse(gfx, me, pageProps, origin));
			}
			if (anyAboveRoot) {
				boundary.Union(ACDrawEllipse(gfx, me, pageProps, origin, true));
			}
			return boundary;
		}
