...
namespace RadialReview.Accessors {
	...
	public class SupportAccessor {
		...
		public static List<TooltipViewModel> GetTooltips(ISession s, string userId, string path) {
			//using (var s = HibernateSession.GetCurrentSession()) {
			//    using (var tx = s.BeginTransaction()) {
			...
			var seenQ = s.QueryOver<TooltipSeen>().Where(x => x.UserId == userId).Select(x => x.TipId).Future<long>();

			var tooltipsQ = s.CreateQuery("from TooltipTemplate t where :path like t.UrlSelector")
				.SetParameter("path", path)
				.Future<TooltipTemplate>();

			var tooltips = tooltipsQ.ToList();
			var seen = seenQ.ToList();
