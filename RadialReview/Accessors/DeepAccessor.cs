...
namespace RadialReview.Accessors {
	public class DeepAccessor : BaseAccessor {
		...
		public class Users {
			...
			public static List<long> GetSubordinatesAndSelf(UserOrganizationModel caller, long userId, PermissionType? type = null, bool includeTempUsers = true) {
				using (var s = HibernateSession.GetCurrentSession()) {
					using (var tx = s.BeginTransaction()) {
			...
			public static List<long> GetSubordinatesAndSelf(ISession s, UserOrganizationModel caller, long userId, PermissionType? type = null, bool includeTempUsers = true) {
				return GetSubordinatesAndSelf(s, PermissionsUtility.Create(s, caller), userId, type, includeTempUsers: includeTempUsers).ToList();
			}

			public static IEnumerable<long> GetSubordinatesAndSelf(ISession s, PermissionsUtility perms, long userId, PermissionType? type = null, bool excludeSelf = false, bool includeTempUsers = true) {
...
#pragma warning restore CS0618 // Type or member is obsolete
		...
		public static List<AccountabilityNode> GetDirectReportsAndSelf(ISession s, PermissionsUtility perms, long forNodeId) {
			...
			var list = s.QueryOver<AccountabilityNode>()
				...
				.Fetch(x => x.User.TempUser).Eager
				.Fetch(x => x.User.Cache).Eager
				.Fetch(x => x.AccountabilityRolesGroup).Eager
				.Fetch(x => x.AccountabilityRolesGroup.Position).Eager
				.List().ToList();
