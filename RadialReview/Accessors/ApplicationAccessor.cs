...
namespace RadialReview.Accessors {
	public class ApplicationAccessor : BaseAccessor {
		...
		public static Boolean EnsureApplicationExists() {
			...
			if (Config.ShouldUpdateDB("EnsureApplicationExists")) {
				...
				AdminOrgs().GetAwaiter().GetResult();
		...
		public static async Task AdminOrgs() {
			...
			if (Config.IsLocal()) {
				...
				if (shouldRun) {
					...
					await UserAccessor.CreateUser(UserManager, user, Guid.NewGuid().ToString());
					using (var s = HibernateSession.GetCurrentSession()) {
						var pp = PaymentAccessor.GetPlanType("professional");

						var res = await new OrganizationAccessor().CreateOrganization(s, user, pp, DateTime.UtcNow, new OrgCreationData() {
		...
		public static List<long> AllowedPhoneNumbers = new List<long>(){}; //REPLACE_ME
