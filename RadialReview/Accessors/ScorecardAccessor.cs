...
namespace RadialReview.Accessors {
	...
	public class MeasurableBuilder {
		...
		private MeasurableBuilder(
			string message, decimal goal, UnitType unitType, LessGreater goalDirection,
			long accountableUserId, long? adminUserId, decimal? alternateGoal, long? templateItemId,
			bool showCumulative, DateTime? cumulativeRange,
			bool showAverage, DateTime? averageRange,
			DateTime? now = null) {
		...
		public static MeasurableBuilder Build(
			string message, long accountableUserId, long? adminUserId = null,
			UnitType type = UnitType.None, decimal goal = 0, LessGreater goalDirection = LessGreater.GreaterThan, decimal? alternateGoal = null,
			bool showCumulative = false, DateTime? cumulativeRange = null,
			bool showAverage = false, DateTime? averageRange = null,
			DateTime? now = null) {
			return new MeasurableBuilder(message, goal, type, goalDirection, accountableUserId, adminUserId, alternateGoal, null, showCumulative, cumulativeRange, showAverage, averageRange, now);
		}
	...
	public class ScorecardAccessor {
		...
		public static async Task<MeasurableModel> CreateMeasurable(ISession s, PermissionsUtility perms, MeasurableBuilder measurableBuilder) {
			...
			s.Save(m);
			var scores = await _GenerateScoreModels_AddMissingScores_Unsafe(s, new DateRange(DateTime.UtcNow.AddDays(-120), DateTime.UtcNow.AddDays(7)), new List<long> { m.Id }, new List<ScoreModel>());

			await HooksRegistry.Each<IMeasurableHook>((ses, x) => x.CreateMeasurable(ses, perms.GetCaller(), m, scores));
			return m;
		}

		public static async Task CreateMeasurableHelper(UserOrganizationModel caller, AngularMeasurable model, decimal? lower, decimal? upper, bool? enableCumulative, DateTime? cumulativeStart, bool? enableAverage, DateTime? averageStart, string formula, long[] recurrenceIds) {
			var builder = MeasurableBuilder.Build(
				model.Name,
				model.Owner.Id,
				model.Admin.NotNull(x => (long?)x.Id),
				model.Modifiers ?? UnitType.None,
				lower ?? model.Target ?? 0,
				model.Direction ?? LessGreater.GreaterThan,
				upper ?? model.AltTarget,
			...
			foreach (var i in recurrenceIds ?? new long[0]) {
				await L10Accessor.AttachMeasurable(caller, i, created.Id);
			}
		...
		private static async Task<List<ScoreModel>> _GenerateScoreModels_AddMissingScores_Unsafe(ISession s, IEnumerable<Tuple<DateTime, long>> weekMeasurables, List<ScoreModel> existing) {
			...
			foreach (var wm in weekMeasurables.Where(x => x.Item1 >= MINIMUM_SCORE_WEEK)) {
				var week = wm.Item1.StartOfWeek(DayOfWeek.Sunday);
				var mid = wm.Item2;
            ...
            using (var s = HibernateSession.GetCurrentSession())
            {
                using (var tx = s.BeginTransaction())
                {
                    ...
                    var q = s.QueryOver<L10Recurrence.L10Recurrence_Measurable>()
                        .JoinAlias(x => x.Measurable, () => measurable).Where(x => x.L10Recurrence.Id == recurrenceId && measurable.AccountableUserId == userId);
		...
		public static async Task DeleteMeasurable(UserOrganizationModel caller, long measurableId) {
			using (var s = HibernateSession.GetCurrentSession()) {
				using (var tx = s.BeginTransaction()) {
					var perms = PermissionsUtility.Create(s, caller);
					await DeleteMeasurable(s, perms, measurableId);
					tx.Commit();
					s.Flush();
				}
		...
		public static async Task DeleteMeasurable(ISession s, PermissionsUtility perms, long measurableId) {
			...
			m.DeleteTime = DateTime.UtcNow;//Edited. Was 'null'
			m.Archived = true;
			s.Update(m);
		...
		public static void UndeleteMeasurable(UserOrganizationModel caller, long measurableId) {
			using (var s = HibernateSession.GetCurrentSession()) {
				using (var tx = s.BeginTransaction()) {
					...
					var rrs = s.QueryOver<L10Recurrence.L10Recurrence_Measurable>()
						.Where(x => x.Measurable.Id == measurableId && x.DeleteTime > deleteTime.Value.AddMinutes(-3) && x.DeleteTime < deleteTime.Value.AddMinutes(3) && x.DeleteTime != null)
						.List().ToList();
					...
					var mrs = s.QueryOver<L10Meeting.L10Meeting_Measurable>()
						.Where(x => x.Measurable.Id == measurableId && x.DeleteTime > deleteTime.Value.AddMinutes(-3) && x.DeleteTime < deleteTime.Value.AddMinutes(3) && x.DeleteTime != null)
						.List().ToList();
					...
					//await HooksRegistry.Each<IMeasurableHook>((ss, x) => x.(ss, rock));
					tx.Commit();
					s.Flush();
				}
			}
		...
		public Csv Listing(UserOrganizationModel caller, long organizationId, DateRange range) {
			using (var s = HibernateSession.GetCurrentSession()) {
				using (var tx = s.BeginTransaction()) {
		...
		public static async Task<bool> UserScorecardCsv(UserOrganizationModel caller, long userId, DateRange range, FileOrigin origin) {
			long fileId;
			using (var s = HibernateSession.GetCurrentSession()) {
				using (var tx = s.BeginTransaction()) {
					...
					fileId = FileAccessor.SaveGeneratedFilePlaceholder_Unsafe(
						s, caller.Id,
						"Personal Scorecards", "csv",
						"generated " + DateTime.UtcNow.AddMinutes(-caller.GetTimezoneOffset()),
						origin,
						FileOutputMethod.Download,
						null,
						TagModel.Create("Scorecard"),
						TagModel.Create<UserOrganizationModel>(userId, "Scorecard")
					);
					tx.Commit();
					s.Flush();
				}
			...
			Scheduler.Enqueue(() => UserScorecardCsv_Hangfire(caller.Id, fileId, userId, range, FileNotification.NotifyCaller(caller)));
			return true;
		}
		...
		public static async Task<bool> UserScorecardCsv_Hangfire(long callerId, long fileId, long userId, DateRange range, FileNotification notify) {
			...
			var csv = ExportAccessor.GenerateScorecardCsv("Scorecard", data, range, caller.GetTimezoneOffset());
			using (var s = HibernateSession.GetCurrentSession()) {
				using (var tx = s.BeginTransaction()) {
					await FileAccessor.Save_Unsafe(s, fileId, csv.ToCsv().ToStream(), notify);
					tx.Commit();
					s.Flush();
				}
			}
		...
		public static async Task SetFormula(ISession s, PermissionsUtility perms, long measurableId, string formula) {
			...
			if ((formula ?? "").Trim() == (measurable.Formula ?? "")) {
				return;
			}
		...
		public static async Task UpdateAllCalculatedScores_Unsafe(ISession s, MeasurableModel measurable) {
			...
			var allMeasurableScoresLookup = allMeasurableScores
				.GroupBy(x => x.ForWeek)
				.ToDictionary(x => x.Key, x => x.OrderBy(y => y.Measured ?? decimal.MinValue).Last());
		...
		public static async Task UpdateCalculatedScores_FromUpdatedScore_Unsafe(ISession s, List<ScoreModel> scores) {
			...
			var scoreLookup = actualData
				...
				.ToDefaultDictionary(
					x => x.Key,
					x => x.ToDefaultDictionary(y => y.week, y => (double?)y.measured, y => null),
					x => new DefaultDictionary<long, double?>(y => null)
				);
			...
			foreach (var u in cellsToUpdate) {
				var parsed = variablesLookup[u.measurableId];
				var variables = GetVariables(parsed).Distinct(x => x.Variable).ToDictionary(x => x.Variable, x => x);
				var theScore = actualScores.Where(x => TimingUtility.GetWeekSinceEpoch(x.ForWeek) == u.weekId && x.MeasurableId == u.measurableId).OrderBy(x => x.Measured ?? decimal.MinValue).LastOrDefault();
		...
		public static CreateMeasurableViewModel BuildCreateMeasurableVM(UserOrganizationModel caller, dynamic ViewBag, long? selectedUserId, long? selectedRecurId, bool hideMeetings, List<SelectListItem> potentialUsers = null) {

			if (potentialUsers == null) {
				potentialUsers = SelectListAccessor.GetUsersWeCanCreateMeaurableFor(caller, caller.Organization.Id, x => x.Id == selectedUserId);
			}
			...
			var meetings = SelectListAccessor.GetL10RecurrenceAdminable(caller, caller.Id, x => x.Id == selectedRecurId);
            ...
            using (var s = HibernateSession.GetCurrentSession())
            {
                using (var tx = s.BeginTransaction())
                {
                    ...
                    return s.QueryOver<L10Recurrence.L10Recurrence_Measurable>()
                        ...
                        .List<object[]>()
                    .Select(x => new NameId((string)x[1], (long)x[0])).ToList();
                }
            }          
        }
