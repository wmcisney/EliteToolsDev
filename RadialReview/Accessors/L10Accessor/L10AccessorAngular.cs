...
namespace RadialReview.Accessors {
	public partial class L10Accessor : BaseAccessor {
		...
		public static async Task<AngularRecurrence> GetOrGenerateAngularRecurrence(ISession s, PermissionsUtility perms, long recurrenceId, bool includeScores = true, bool includeHistorical = true, bool fullScorecard = true, DateRange range = null, bool forceIncludeTodoCompletion = false, DateRange scorecardRange = null, bool includeCreatedBy = false) {
				...
				using (var tx = s.BeginTransaction()) {
					using (var rt = RealTimeUtility.Create(connectionId)) {
						...
						if (model.Type == typeof(AngularIssue).Name) {
							...
							await DetachMeasurable(s, perms, rt, recurrenceId, model.Id, true);
