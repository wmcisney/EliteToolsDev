...
namespace RadialReview.Accessors {
	...
	public partial class L10Accessor : BaseAccessor {
		...
		public static async Task RemoveRock(UserOrganizationModel caller, long recurrenceId, long rockId, bool archiveWhenNotInMeetings = true) {
			using (var s = HibernateSession.GetCurrentSession()) {
				using (var tx = s.BeginTransaction()) {
					using (var rt = RealTimeUtility.Create()) {
						var perms = PermissionsUtility.Create(s, caller);

						await RemoveRock(s, perms, rt, recurrenceId, rockId, archiveWhenNotInMeetings);
		...
		public static async Task RemoveRock(ISession s, PermissionsUtility perm, RealTimeUtility rt, long recurrenceId, long rockId, bool archiveWhenNotInMeetings = true) {
		...
		public static async Task SetVtoRock(UserOrganizationModel caller, long recurRockId, bool vtoRock) {
			using (var s = HibernateSession.GetCurrentSession()) {
				using (var tx = s.BeginTransaction()) {
					var perms = PermissionsUtility.Create(s, caller);
					await SetVtoRock(s, perms, recurRockId, vtoRock);
					tx.Commit();
					s.Flush();
				}
			}
