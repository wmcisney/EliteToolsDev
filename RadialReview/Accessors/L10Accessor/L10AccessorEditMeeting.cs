...
#pragma warning restore CS0618 // Type or member is obsolete
		...
		public static async Task SetSharedVTOVision(UserOrganizationModel caller, long organizationId, long recurrenceId) {
			using (var s = HibernateSession.GetCurrentSession()) {
				using (var tx = s.BeginTransaction()) {
					var perms = PermissionsUtility.Create(s, caller);
					await SetSharedVTOVision(s, perms, organizationId, recurrenceId);
					tx.Commit();
					s.Flush();
				}
			}
		...
		public static async Task UnsetSharedVTOVision(ISession s, PermissionsUtility perms, long organizationId) {
			await SetSharedVTOVision(s, perms, organizationId, 0);
		}
		...
		public static async Task SetSharedVTOVision(ISession s, PermissionsUtility perms, long organizationId, long recurrenceId) {
			...
			if (recurrenceId != 0) {
				...
				perms.CanEdit(PermItem.ResourceType.L10Recurrence, recurrenceId, exceptionMessage: "Cannot share V/TO from selected meeting (" + recurrenceId + ")");
			}
			...
			foreach (var e in existing) {
				perms.CanEdit(PermItem.ResourceType.L10Recurrence, e.Id, exceptionMessage: "Cannot unshare V/TO from existing meeting (" + e.Id + ")");
				e.ShareVto = false;
				s.Update(e);
			}
			if (found != null) {
