...
namespace RadialReview.Accessors {
	public class UserTemplateAccessor {
		...
		public static async Task AddUserToTemplate(UserOrganizationModel caller, long templateId, long userId, bool forceJobDescription) {
			using (var s = HibernateSession.GetCurrentSession()) {
				using (var tx = s.BeginTransaction()) {
					var p = PermissionsUtility.Create(s, caller)
						.EditTemplate(templateId)
						.ManagesUserOrganization(userId, false);
					await _AddUserToTemplateUnsafe(s, p, caller.Organization, templateId, userId, forceJobDescription);
