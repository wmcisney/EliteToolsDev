...
namespace RadialReview.Controllers {
	...
	public class SchedulerController : BaseController {
		...
		public async Task<ActionResult> EmailTodos(int currentTime, CancellationToken ct, int divisor = 13, int remainder = 0, int sent = 0, string error = null, double duration = 0, bool mockEmails = false) {
			...
			Scheduler.Enqueue(() => TodoEmailsScheduler.TodoEmailHelpers.SendTodoReminder_Hangfire(currentTime, nowUtc, mockEmails));
		...
		[Access(AccessLevel.Any)]
		public async Task<JsonResult> CloseMeetings() {
			List<long> ids;
			using (var s = HibernateSession.GetCurrentSession()) {
				using (var tx = s.BeginTransaction()) {
					var hours = s.GetSettingOrDefault(Variable.Names.CLOSE_MEETING_AFTER, () => 16.0);
					var time = DateTime.UtcNow.AddHours(-hours);
					ids = s.QueryOver<L10Meeting>().Where(x => x.DeleteTime == null && x.CompleteTime == null && x.CreateTime < time).Select(x => x.L10RecurrenceId).List<long>().Distinct().ToList();
				}
			}

			foreach (var id in ids) {
				Scheduler.Enqueue(() => TaskAccessor.CloseMeeting(id));
			}

			return Json(ids.Count(), JsonRequestBehavior.AllowGet);
		}
		...
		public async Task<JsonResult> Reschedule(CancellationToken ct) {
			//AWS LAMBDA CALLS ME			
			Scheduler.Enqueue(() => TaskAccessor.EnqueueTasks(DateTime.UtcNow));
			return Json(true, JsonRequestBehavior.AllowGet);
		}
