...
namespace RadialReview.Controllers {
    public partial class UploadController : BaseController {
            ...
            try {
                ...
                if (fileType == FileType.CSV && (users != null || details != null || duedate != null)) {
                    ...
                    if (duedate != null) {
                        ...
                        var duedates = duedateRect.GetArray1D(csvData, x => {
							DateTime d = now.AddDays(7);
							DateTime.TryParse(x, out d);
							if (d < new DateTime(1980, 1, 1)) {
								d = now.AddDays(7);
							}
							return d;
						});
        ...
        [Access(AccessLevel.UserOrganization)]
        public async Task<FileResult> SampleTodos() {

            var builder = new StringBuilder();
            builder.AppendLine("To-dos,Due,Who,Details");
            builder.AppendLine($"Contact Allison for referral,{DateTime.UtcNow.AddDays(7).ToString("MM/dd/yyyy")},{GetUser().GetName()}, Get list of preferred providers.");

            var bytes = Encoding.UTF8.GetBytes(builder.ToString());
            Response.AddHeader("Content-Disposition", "attachment;filename=example_todos.csv");
        ...
        public async Task<JsonResult> SubmitTodos(FormCollection model) {
            ...
            try {
                ...
                return Json(ResultObject.CreateRedirect("/l10/wizard/" + recurrence +"?noheading=" + noTitleBar + "#Todos", message));
            } catch (Exception e) {
                //e.Data.Add("AWS_ID", path);
                throw new Exception(e.Message + "[" + path + "]", e);
            }
