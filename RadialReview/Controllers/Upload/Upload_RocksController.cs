...
namespace RadialReview.Controllers {
    public partial class UploadController : BaseController {
		...
		[Access(AccessLevel.UserOrganization)]
		public async Task<FileResult> SampleRocks() {
			
			var builder = new StringBuilder();
			builder.AppendLine("Rocks,Who,Due,Details");
			builder.AppendLine($"Close the ABC Co. Deal,{GetUser().GetName()},{DateTime.UtcNow.AddDays(90).ToString("MM/dd/yyyy")},Terms agreed on and contract has been signed.");

			var bytes = Encoding.UTF8.GetBytes(builder.ToString());
			Response.AddHeader("Content-Disposition", "attachment;filename=example_rocks.csv");
		...
		public async Task<JsonResult> SubmitRocks(FormCollection model) {
			...
			try {
				...
				using (var s = HibernateSession.GetCurrentSession()) {
					using (var tx = s.BeginTransaction()) {
						...
						foreach (var m in rocks) {
							...
							await L10Accessor.AttachRock(s, perms, recurrence, rock.Id, false, AttachRockType.Create);
				...
				return Json(ResultObject.CreateRedirect("/l10/wizard/" + recurrence + "?noheading=" + noTitleBar + "#Rocks", "Uploaded Rocks"));
			} catch (Exception e) {
				//e.Data.Add("AWS_ID", path);
				throw new Exception(e.Message + "[" + path + "]", e);
			}
