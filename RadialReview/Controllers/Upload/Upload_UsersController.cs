...
namespace RadialReview.Controllers {
	public partial class UploadController : BaseController {
		...
		[Access(AccessLevel.UserOrganization)]
		public async Task<FileResult> SampleUsers() {

			var builder = new StringBuilder();
			var position = "Position";
			try {
				position = GetUser().Positions.FirstOrDefault().NotNull(x => x.Position.NotNull(y => y.GetName()));
			} catch (Exception) {
			}

			builder.AppendLine("First Name,Last Name,Email,Position,Manager First Name,Manager Last Name");
			builder.AppendLine($"{GetUser().GetFirstName()},{GetUser().GetLastName()},{GetUser().GetEmail()},{position},ManagerFirst,ManagerLast");

			var bytes = Encoding.UTF8.GetBytes(builder.ToString());
			Response.AddHeader("Content-Disposition", "attachment;filename=example_users.csv");
...
#pragma warning restore CS0618 // Type or member is obsolete
				...
				var uploadedUsers = new List<UserOrganizationModel>();
				...
				foreach (var m in incs) {
					...
					var existingUser = existingUsers.FirstOrDefault(x => x.EmailAtOrganization == email);
					...
					try {
						...
						if (attendee == null) {
							...
							var nodes = AccountabilityAccessor.GetNodesForUser(GetUser(), attendee.Id);
							if (nodes.Count == 0) {
								uploadedUsers.Add(attendee);
							}
						}
						try {
							await L10Accessor.AddAttendee(GetUser(), recurrence, attendee.Id);
				...
				AccountabilityAccessor._FinishUploadAccountabilityChart(GetUser(), uploadedUsers, managerLookup, errors);
				//return Json(ResultObject.CreateRedirect("/l10/wizard/" + recurrence + "#Attendees", "Uploaded Users"));
				return Json(ResultObject.CreateRedirect("/l10/wizard/" + recurrence + "?noheading=" + noTitleBar + "#Attendees", "Uploaded Users"));
			} catch (Exception e) {
				throw new Exception(e.Message + "[" + path + "]", e);
			}
		}
