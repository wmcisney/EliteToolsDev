...
namespace RadialReview.Controllers {
	public partial class L10Controller : BaseController {
		...
		public async Task<ActionResult> Load(long id, string connection, string page = null) {
			...
			switch (page) {
				...
				case "startmeeting":
					if (recurrence.MeetingInProgress == null) {
						return await StartMeeting(model, true);
					} else {
						page = "";
						break;
					}
			...
			try {
				...
				if (e.MeetingExceptionType == MeetingExceptionType.Unstarted) {
					...
					return await StartMeeting(model, false);
				}
		...
		[HttpGet]
		[Access(AccessLevel.UserOrganization)]
		public async Task<PartialViewResult> StartMeeting(L10MeetingVM model, bool start) {
			try {
				var me = model.Recurrence.NotNull(x => x._DefaultAttendees.FirstOrDefault(y => y.User.Id == GetUser().Id));
			...
			await L10Accessor.NotifyOthersOfMeeting(GetUser(), model.Recurrence.Id);
		...
		private PartialViewResult Segue(L10MeetingVM model) {
			ViewBag.Subheading = ViewBag.Subheading ?? CustomizeAccessor.GetSpecificCustomization(GetUser(), GetUser().Organization.Id, CUSTOMIZABLE.Segue_Subheading, "Share good news from the last 7 days.<br/> One personal and one business.");
...
#pragma warning restore CS1998 // Async method lacks 'await' operators and will run synchronously
		...
		private PartialViewResult Conclusion(L10MeetingVM model, FormCollection form, bool start) {
			...
			if (model.MeetingStart != null && (DateTime.UtcNow - model.MeetingStart.Value) < TimeSpan.FromMinutes(12)) {
				model.CloseTodos = false;
				model.CloseHeadlines = false;
				model.SendEmailRich = ConcludeSendEmail.None;
				ViewBag.AdditionalMessage = "It looks like you're just previewing this meeting. We won't close out your to-dos or headlines, nor send a summary.";
			}
		...
		public async Task<ActionResult> Conclusion(L10MeetingVM model, FormCollection form, string connectionId = null) {
			...
			if (ModelState.IsValid) {
				...
				if (ModelState.IsValid) {
					await L10Accessor.ConcludeMeeting(GetUser(), model.Recurrence.Id, ratingValues, model.SendEmailRich, model.CloseTodos, model.CloseHeadlines, connectionId, model.UseNewEmailFormat);
		...
		public async Task<ActionResult> EditRanting(L10MeetingVM model, FormCollection form, string connectionId = null) {
			...
			if (ModelState.IsValid) {
				...
				if (ModelState.IsValid) {
					...
					return Json(new { data = model1.AverageRating>0?model1.AverageRating:0, response = "success" }, JsonRequestBehavior.AllowGet);
				}
			}
