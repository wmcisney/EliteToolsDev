...
namespace RadialReview.Controllers {
	public class UserController : BaseController {
		...
		public ActionResult UpdateCache(long id) {
			var u = GetUser();
			UserLookup res;
		...
		public PartialViewResult AddModal(
			...
			bool forceNoSend = false, long? recurrenceId = null) {

			if (!PermissionsAccessor.IsPermitted(GetUser(), x => x.CanAddUserToOrganization(GetUser().Organization.Id)))
				throw new PermissionsException("You are not permitted to create users.");

			var model = UserAccessor.BuildCreateUserVM(GetUser(), ViewBag, recurrenceId, managerId, name, isClient, managerNodeId, forceManager, hideIsManager, hidePosition, nodeId, hideEvalOnly, forceNoSend);
		...
		public async Task<JsonResult> EditUserDetails(EditUserDetailsVM model, HttpPostedFileBase image) {
			...
			var message = image == null ? "User updated." : "Image updated.";
			if (System.Web.HttpContext.Current.Request.Files.AllKeys.Any()) {
			}
			return Json(ResultObject.Create(model, message).ForceRefresh());
		}
...
#pragma warning restore CS0618 // Type or member is obsolete
		...
		public async Task<JsonResult> PositionModal(UserPositionViewModel model) {
			...
			await _PositionAccessor.AddPositionToUser(GetUser(), model.UserId, model.PositionId);
