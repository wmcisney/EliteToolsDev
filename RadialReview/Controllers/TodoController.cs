...
namespace RadialReview.Controllers {
	public class TodoController : BaseController {
		...
		[Access(AccessLevel.UserOrganization)]
		public PartialViewResult CreateTodoRecurrence(long? id = null, long? recurrenceId = null, long? recurrence = null, string todo = null, string details = null, long? modelId = null, string modelType = null, long? meetingId = null, long? meeting = null, TodoContext context = TodoContext.General,bool showRecurrences=false) {
			...
			ViewBag.ShowRecurrenceDropdown = showRecurrences;
			...
			if (ViewBag.ShowRecurrenceDropdown == true) {
				var meetings = L10Accessor.GetVisibleL10RecurrencesVM(GetUser(), GetUser().Id, false)
		...
		public ActionResult EditModal(long id, bool showMeeting = true) {
			...
			if (id > 0) {
				...
				var meetings = SelectListAccessor.GetL10RecurrenceEditable(GetUser(), GetUser().Id, x => x.Id == todo.ForRecurrenceId, true);

				ViewBag.Originating = "";
				ViewBag.ShowMeeting = showMeeting;

				meetings.Insert(0, new SelectListItem() {
					Text = "Personal To-do list",
					Value = "-2" // Personal todo list
				});
		...
		*/
		...
		public async Task<PartialViewResult> CreateScorecardTodo(long meeting, long recurrence, long measurable, long score, long? accountable = null) {
			...
			if (accountable.HasValue && !people.Any(x => x.Id == accountable)) {
				accountable = null;
			}
            var accountabilityUserId = s.AccountableUser != null ? s.AccountableUser.Id : recur.GetDefaultTodoOwner(GetUser());
            if (!people.Any(x => x.Id == accountabilityUserId))
                accountabilityUserId = recur.GetDefaultTodoOwner(GetUser());
            var model = new ScoreCardTodoVM(recur.GetDefaultTodoOwner(GetUser()), GetUser()) {
				...
				MeetingId = meeting,
				RecurrenceId = recurrence,
				AccountabilityId = new[] { accountabilityUserId },
				PossibleUsers = people.Select(x => new AccountableUserVM() {
					id = x.Id,
					imageUrl = x.ImageUrl(true, ImageSize._32),
					name = x.GetName()
		...
		public async Task<PartialViewResult> CreateRockTodo(long meeting, long recurrence, long? rock = null, long? rockId = null, long? accountable = null) {
            ...
            var accountabilityUserId = s.ForRock.AccountableUser != null ? s.ForRock.AccountableUser.Id : recur.GetDefaultTodoOwner(GetUser());
            if (!people.Any(x => x.Id == accountabilityUserId))
                accountabilityUserId = recur.GetDefaultTodoOwner(GetUser());
		...
		public async Task<PartialViewResult> CreateTodoFromHeadline(long meeting, long recurrence, long headline, long? accountable = null) {
            ...
            var accountabilityUserId = s.Owner != null ? s.Owner.Id : recur.GetDefaultTodoOwner(GetUser());
            if (!people.Any(x => x.Id == accountabilityUserId))
                accountabilityUserId = recur.GetDefaultTodoOwner(GetUser());
		...
		[Access(AccessLevel.UserOrganization)]
		public async Task<PartialViewResult> CreateTodoFromIssue(long recurrence_issue, long recurrence, long? meeting = null) {
			if (meeting != null) {
				PermissionsAccessor.Permitted(GetUser(), x => x.ViewL10Meeting(meeting.Value));
			}
            var issueRecur = IssuesAccessor.GetIssue_Recurrence(GetUser(), recurrence_issue);
            var i = issueRecur.Issue;
			var recur = L10Accessor.GetL10Recurrence(GetUser(), recurrence, LoadMeeting.True());

			var people = recur._DefaultAttendees.Select(x => x.User).ToList();
			people.Add(GetUser());
			people = people.Distinct(x => x.Id).ToList();            
            var accountabilityUserId = issueRecur.Owner != null ? issueRecur.Owner.Id : recur.GetDefaultTodoOwner(GetUser());
            if (!people.Any(x => x.Id == accountabilityUserId))
                accountabilityUserId = recur.GetDefaultTodoOwner(GetUser());
