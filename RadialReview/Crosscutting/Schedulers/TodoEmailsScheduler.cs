...
namespace RadialReview.Crosscutting.ScheduledJobs {
	public class TodoEmailsScheduler{
		...
		public class TodoEmailHelpers {
			...
			public static async Task<int> SendTodoReminder_Hangfire(int currentTime,DateTime nowUtc,bool mockEmails) {

				var unsent = new List<Mail>();
				using (var s = HibernateSession.GetCurrentSession()) {
					using (var tx = s.BeginTransaction()) {
						await TodoEmailsScheduler.TodoEmailHelpers._ConstructTodoEmails(currentTime, unsent, s, nowUtc);
						tx.Commit();
						s.Flush();
					}
				}
			...
			public static async Task _ConstructTodoEmails(int currentTime, List<Mail> unsent, ISession s, DateTime nowUtc/*, int divisor, int remainder*/) {
				Dictionary<string, List<TinySchedulerTodo>> dictionary = _GetAllTodosForSendTime(s, currentTime, nowUtc);
			...
			public static Dictionary<string, List<TinySchedulerTodo>> _GetAllTodosForSendTime(ISession s, int currentTime, DateTime nowUtc) {
				...
				var todos = _QueryTodo(s, currentTime, /*divisor, remainder,*/ rangeLow, rangeHigh, nextWeek);
			...
			public static List<TinySchedulerTodo> _QueryTodo(ISession s, int currentTime,/* long divisor, long remainder,*/ DateTime rangeLow, DateTime rangeHigh, DateTime nextWeek) {
				...
				var query = s.QueryOver<TodoModel>()
								...
								.Where(x => ((rangeLow <= x.DueDate && x.DueDate <= rangeHigh) || (x.CompleteTime == null && x.DueDate <= nextWeek)) && x.DeleteTime == null && org.DeleteTime==null && accUser.DeleteTime==null);
			...
			public static async Task _ConstructTodoEmail(int currentTime, List<Mail> unsent, DateTime nowUtc, List<TinySchedulerTodo> userTodos) {
				...
				var shouldSend = userTodos.Count(x => x.DueDate.Date >= nowLocal.Date.AddDays(-30) && x.CompleteTime == null);
				...
				if (subject != null && shouldSend > 0) {
					...
					try {
						...
						int a = 0;
						log.Error("ConstructTodoEmail Error",e);
					}
				}
			}
		}
