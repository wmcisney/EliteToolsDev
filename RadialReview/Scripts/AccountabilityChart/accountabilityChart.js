...
var collapseAll = function (n) {
	var rs = angular.element("[ng-controller]").scope();
	rs.$apply(function () {
		rs.$emit("CollapseAllNodes", n);
	});
...
var setLevel = function (n) {
	...
	$(".current-level").html(" (" + n + ") ");

	setTimeout(function () {
		clearAlerts();
		if (n == 1)
			showAlert("Showing only the top level");
		else
			showAlert("Showing only the top " + n + " levels");
	}, 450);
...
var recenter = function () {
	...
	rs.$apply(function () {
		var offset = { x: -280 / 2 };
		if (rs.showPanel != true) {
			offset = {};
		}
		if (rs.selected == null) {
			rs.$emit("CenterRootNode", 1, 400, offset);
		} else {
			rs.$emit("CenterNode",rs.selected, 1, 400, offset);
        }
	});
}

var initRecenter = function () {
    var rs = angular.element("[ng-controller]").scope();
    rs.$apply(function () {
        var offset = { x: -280 / 2 };
        if (rs.showPanel != true) {
        ...
        rs.$emit("CenterRootNode", 0, 0, offset);
    });
}

var iscompact = false;
var compactify = function () {
	iscompact = !(iscompact || false);

	angular.element($("[ng-controller]")).scope().$apply(function () {
		angular.element($("[ng-controller]")).scope().compact = this.iscompact;
	});
	$("body").toggleClass("is-compact", iscompact);


	if (iscompact && !this.message) {
		this.message = true;
		showAlert("Please note, compact mode has limited functionality.", 2000);
		setTimeout(function () {
			showAlert("Reorganizing your Accountablity Chart and expanding seats are disabled in this mode.", 4000);
		}, 3000);
	}
...
var genPdf = function (val, nodeId) {
	...
	var fields = [
		...
		{ text: "Compact Chart", name: "compact", type: "checkbox", value: iscompact, onchange: function () { compactify($(this).is(":checked")); } },
	...
	showModal({
		...
		close: function () {
			angular.element($("[ng-controller]")).scope().$apply(function () {
				angular.element($("[ng-controller]")).scope().compact = iscompact;
			});
		}
	});
};
