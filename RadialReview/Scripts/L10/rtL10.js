...
$(function () {
	...
	var setup = function (i) {
		...
		RealTime.client.removeVideoProvider = removeVideoProvider;
		...
		RealTime.client.setIssueMoveLocation = setIssueMoveLocation;
...
function initConnection() {
	...
	console.log("[rtl10] initConnection");
...
function rejoin(callback) {
	...
	console.log("[rtl10] rejoin (" + window.recurrenceId+")");
...
function userEnterMeeting(connection) {
	...
	if (!Enumerable.from(userEnterMeeting.existing).any(function (x) { return x.id == id; })) {
		...
		var n = $(".online-component-container .user-picture").length;
		$(".online-component-container .online-count").html("(" + n + ")");
	}
