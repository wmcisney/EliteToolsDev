...
$(function () {
	CheckOffIssue = Undo.Command.extend({
		constructor: function (row) {
			...
			if ($(this.issueRow).hasClass("selected")) {
				...
				$(this.issueRow).removeClass("selected");
				currentIssuesDetailsId = null;
	...
	$("body").on("click", ".issues-list>.issue-row:not(.undoable)", function (evt) {
		...
		$(detailsContents).append("<div class='button-bar'>" +
			...
			"<span class='issue-move-location'></span>"+
            ...
            "<span class='btn btn-default btn-xs createTodoButton todoModal on-edit-enabled' data-recurrence_issue='" + recurrence_issue + "' data-method='CreateTodoFromIssue' data-meeting='" + window.meetingId + "' data-issue='" + issueId + "' data-recurrence='" + window.recurrenceId + "' ><span class='glyphicon glyphicon-unchecked todoButton'></span> To-Do</span>" +
            "</span>" +
            "</div>" +
...
function constructRow(issue) {
	...
	return '<li class="issue-row dd-item arrowkey undoable-stripped"  data-createtime="' + issue.createtime + '" data-recurrence_issue="' + issue.recurrence_issue + '" data-issue="' + issue.issue + '" data-checked="' + issue.checked + '"  data-message="' + issue.message + '"  data-details="' + issue.details + '"  data-owner="' + issue.owner + '" data-accountable="' + issue.accountable + '"  data-priority="' + issue.priority + '"  data-rank="' + issue.rank + '" data-awaitingsolve="' + issue.awaitingsolve + ' data-markedforclose="' + issue.markedforclose + '">\n'
		...
		+ "<span class='issue-move-location'></span>\n"
        + '<span class="issuesButton issuesModal_move icon fontastic-icon-forward-1" data-copyto="' + window.recurrenceId + '" data-recurrence_issue="' + issue.recurrence_issue + '" data-method="copymodal" style="padding-right: 5px"></span>\n'
        + '<span class="glyphicon glyphicon-unchecked todoButton issuesButton todoModal" data-recurrence_issue="' + issue.recurrence_issue + '" data-issue="' + issue.issue + '" data-meeting="' + issue.createdDuringMeetingId + '" data-recurrence="' + window.recurrenceId + '" data-method="CreateTodoFromIssue" style="padding-right:5px;"></span>\n'
...
function setIssueMoveLocation(issueRecurId, meetingName) {
	$(".issue-row[data-recurrence_issue='" + issueRecurId + "']").find(".issue-move-location").text(meetingName);
...
function updateModedIssueSolve(issueRecurId, status) {
	...
	row.find(".issue-checkbox").attr("checked", status?"checked":false);
	console.log("checked:" + issueRecurId);
	//undoStack.execute(new CheckOffIssue(this, status));

	//Update Selected
	var issueRow = row.closest(".issue-row");
	if ($(issueRow).hasClass("selected") && status) {
		$(issueRow).addClass("wasSelected");
		...
		if ($(window).width() > modalWidth) {
			...
			if (next.length == 0 || next.find("input:checked").length > 0) {
				var next = $(issueRow);
				while (true) {
					next = next.prev();
					if (next.find("input:checked").length > 0)
			...
			setTimeout(function () {
				try {
					if (next.length && next.find("input:checked").length == 0) {
						$(next).click();
					} else {
						$("#issueDetails").html("");
					}
...
function refreshCurrentIssueDetails() {
	...
	if ($(window).width() > modalWidth) {
		setTimeout(function () {
			var found = $(".issue-row[data-recurrence_issue=" + currentIssuesDetailsId + "]")
