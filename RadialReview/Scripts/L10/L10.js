...
function setFollowLeader(val) {
	...
	if (val == false) {
		clearAlerts();
		setTimeout(function () {
			showAlert("You can now change pages freely.")
		}, 600);
	}
}

function unfollowLeader() {
	$('.switch').bootstrapSwitch('state', false);
}

function showUnstartedMeetingAlert() {
	clearAlerts();
	showAlert("This meeting has not started yet. <br/>Click <i>Start Meeting</i> to begin.", 7000);
}

function showLeaderAlert() {
	clearAlerts();
	showAlert("You are locked into following the meeting leader. <br/> <a href='#' onclick='unfollowLeader()'><u>Unlock this setting</u></a> or <a href='#' onclick='setLeader()'><u>become the meeting leader</u></a>.",7000);
}
...
function resetClickables() {
	...
	$(".agenda .agenda-items a").attr("onclick", "");

	var showLeaderToggle = !isLeader && meetingStart;
	$(".agenda .follow-leader-section").toggleClass("hidden", !showLeaderToggle);

	if (!meetingStart) {
		$(".agenda .agenda-items a").prop("title", "");
		$(".agenda .agenda-items a").addClass("clickable");
		$(".agenda .agenda-items a").prop("href", "#");
		$(".agenda .agenda-items a").attr("onclick", "showUnstartedMeetingAlert()");
	} else if (isLeader || !followLeader) {
        $(".agenda .agenda-items a").addClass("clickable");
        ...
        if (meetingStart) {
			...
			$(".agenda .agenda-items a").attr("onclick", "showLeaderAlert()");
        }
    }
...
function setupMeeting(_startTime, leaderId) {
	...
	if (isLeader && leaderId != userId) {
		showAlert("You are not longer the meeting leader.");
	}
                ...
                replaceMainWindow(data, function () {
                    ...
                    try {
                        if ($('body').is('.meeting-preview')) {
                            if (type == 'conclude') {
                                $('#exit_preview_meeting_button').hide();
                            }
                            else {
                                $('#exit_preview_meeting_button').show();
                            }
                        }
...
function showLoader() {
	replaceMainWindow("<div class='loader centered loader-fader'><div class='component '><div>Loading</div><img src='/Content/select2-spinner.gif' /></div></div>");
	setTimeout(function () {
		$(".loader.loader-fader").removeClass("loader-fader");
	},1000);
}
...
function setLeader() {
	...
	$.ajax({
		url: "/L10/SetLeader/" + window.recurrenceId,
		success: function () {
			clearAlerts();
			setTimeout(function () {
				showAlert("You are now the meeting leader.")
			}, 600);
		}
	});
...
function callbackPrintPdf() {
    $("#modalForm").unbind("submit");
    $("#modalForm").attr("target", "_blank");
    $("#modalForm").attr("method", "post");
    $("#modalForm").attr("action", "/Quarterly/Printout2/" + window.recurrenceId);
    $("#modalForm").bind("submit", function () {
        $("#modal").modal('hide');
    });
}
...
$(window).on("page-conclude", function () {
    $("#edit_meeting_link").attr("href", "/L10/Wizard/" + window.recurrenceId + "?return=meeting");
