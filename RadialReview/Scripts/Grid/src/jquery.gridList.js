...
(function (factory) {
	...
	DraggableGridList.prototype = {
		...
		_onStart: function (event, ui) {
			...
			this._originalDragPos = newPosition;
		...
		_onDrag: function (event, ui) {
			...
			if (this._dragPositionChanged(newPosition)) {
				...
				if (this._originalDragPos[0] == item.x && this._originalDragPos[1] == item.y) {
					GridList.cloneItems(this._items, this.items);
					console.warn("orignal location (reverting) ", item.x,item.y);
				} else {
					console.warn("different location ", item.x, item.y);
				}
		...
		_snapItemPositionToGrid: function (item) {
			...
			if (this.items.length > 0) {
				for (var i = 0; i < this.items.length; i++) {
					...
					gcf = this._gcf(gcf, this.items[i].h);
				}
			}

			if (!gcf || isNaN(gcf) || gcf < 1) {
				gcf = 1;
			}
			console.info("gcf", gcf);
			...
			var col = Math.round(position.left / this._cellWidth),
				row = Math.round(position.top / (this._cellHeight * gcf)) * gcf;
