(function () {
	...
	var app = angular.module('people').component('peopleAnalyzer', {
		...
		controller: ["$scope", "$element", "$scope", "$http", 'radial', "$log", "$timeout", "$window", function ($scope, $element, $attrs, $http, radial, $log, $timeout, $window) {
			...
			if (this.flat || urlParams.get("flat")=="true") {
				query = query + ((query == "") ? "?" : "&") + "flat=true";
			}
            ...
            $scope.getCompanyValues = function (criteria) {
                return function (item) {
                    var exist = item.SurveyContainerIds.indexOf($scope.functions.selectedSurveyContainer().Id);
                    if (exist !== -1)
                        return item;                                     
                };
            };
