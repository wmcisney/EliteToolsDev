angular.module('radialModule', ["updaterModule"]).factory('radial', ['$window', 'updater', '$http', function ($window, updater, $http) {
    ...
    */
	function radialFactory($scope, options) {
		...
		if (typeof (optionshubName) !== "undefined")
			throw "[RadialModule] HubName deprecated";
		if (typeof (options.hubJoinMethod) !== "undefined")
			throw "[RadialModule] hubJoinMethod deprecated";
		if (typeof (options.hubJoinArgs) !== "undefined")
			throw "[RadialModule] hubJoinArgs deprecated";
		...
		//				console.log("[RadialModule] Rejoin completed.");
		//				if (callback) {
		//					callback();
		//				}
		//				if ($scope.disconnected) {
		//					clearAlerts();
		//					showAlert("Reconnected.", "alert-success", "Success", 1500);
		//				}
		//				$scope.disconnected = false;
		//			});
		//		} else {
		//			console.error("[RadialModule] No rt");
		//		}
		//	} catch (e) {
		//		console.error(e);
		//	}
		...
		if (typeof (rejoin) !== "undefined") {
			debugger;
			throw "[RadialModule] REJOIN DEPRECATED";
		}

		if (typeof (options.hubs) !== "undefined") {
			$window.RealTime.join(options.hubs);
		}
		...
		//	//console.error("[RadialModule] In callback");
		//	$scope.connectionId = rt.id;
		//	rejoin(rt, function () {});
		//});

		//hub.afterStart(function (rt) {
		//	//console.error("[RadialModule] In afterStart");
		//	$scope.connectionId = rt.connectionId;
		//});


		//Construct Updater
		var u = updater($scope, $window.RealTime);
		var addTimestamp = Time.addTimestamp;
		$scope.functions.addTimestamp = addTimestamp;

		$scope.functions.getConnectionId = function () {
			if (window && window.RealTime && window.RealTime.connectionId) {
				return window.RealTime.connectionId;
			}
			return undefined;
		};
		...
		var loadData = function (url, opts) {
			...
			if (isReloading) {
				console.error("[RadialModule] Aborting. Already loading.");
				return;
			}
			var urlType = typeof (url);
			if (urlType === "undefined") {
				console.error("[RadialModule] Aborting. Undefined url. Must be a function or string");
				return;
			}
			...
			if (typeof (url) === "function") {
				...
				if (typeof (url) !== "string") {
					console.error("[RadialModule] Aborting. Url did not resolve to a string.");
					return;
				}
			}
			...
			//Make the call
			isReloading = true;
			console.log("[RadialModule] Loading data: " + url);
			$http({ method: 'get', url: url }).then(function (response) {
				...
				console.log("[RadialModule] Loading complete.");
