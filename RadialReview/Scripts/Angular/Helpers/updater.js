angular.module('updaterModule', []).factory('updater', ["$http", function ($http) {
	...
	function applyUpdate(data, status) {
		console.info("[Updater] Applying update");
		convertDates(data);
		this._preExtend(data, status);
		baseExtend(this.scope.model, [data], true, undefined, undefined, this.scope.model);
		...
		for (var i in model) {
			if (arrayHasOwnIndex(model, i)) {
				...
				if ((model[i] + "").toLowerCase() == find) {
					console.log("[Updater] Builder: " + builder)
					arr.push(model);
				}
	...
	function baseExtend(dst, objs, deep, after, lookupFix, baseModel) {
		var h = dst.$$hashKey;
					...
					if (arrayHasOwnIndex(objs, k)) {
						...
						if ("Lookup" in o) {
							if (dst["Key"] in o["Lookup"]) {
								...
								baseExtend(objs[k], [baseObj], deep, true, true, baseModel);
			...
			try {
				if (typeof (obj["UT"]) !== "undefined") {//new
					if (typeof (dst["UT"]) !== "undefined") {//old
						if (obj["UT"] < dst["UT"]) {
							console.info("[Updater] Update skipped:" + (+obj["UT"]) + "<" + (+dst["UT"]));
							continue;
						}
					}
				}
				...
				console.error("[Updater] UT Error", e);
			}
					...
					if (src.AngularList) {
						...
						if (typeof (lookupFix) !== "undefined" && lookupFix == true) {
							...
							if (src.UpdateMethod == "Add") {
								...
								var addObj = src.AngularList;
								////Check if the added object is a reference. If so, replace it from the table.
								//if (addObj && addObj.length) {
								//	for (var ai = 0; ai < addObj.length; ai++) {
								//		try {
								//			debugger;
								//			if (addObj[ai]._P) {
								//				addObj[ai] = baseModel.Lookup[addObj[ai]["Key"]];
								//			}
								//		} catch (e) {
								...
								console.error("[Updater] UpdateMethod unknown:" + src.UpdateMethod);
							}
						}
					} else {
						...
						if (angular.isArray(dst[key])) {
							...
							if ((typeof (src.Key) !== "undefined" || key == "Lookup") && dst[key].Key == src.Key)
								baseExtend(dst[key], [src], true, true, undefined, baseModel);
							else
								dst[key] = src;
						}
					}
		...
		for (var key in obj) {
			if (arrayHasOwnIndex(obj, key)) {
				...
				if (obj[key] == null) {
					...
					if (isDate !== false) {
						if (key == "FutureDate" && (obj.Type == "AngularThreeYearPicture" || obj.Type == "AngularOneYearPlan" || obj.Type == "AngularQuarterlyRocks")) {
							var futureDate = value;
							var convertedDate = futureDate;
							if (futureDate.indexOf('/Date(') !== -1) {
								futureDate = futureDate.replace("/Date(", "").replace(")/", "");
								convertedDate = new Date(parseInt(futureDate));
							}
							else {
								convertedDate = value;
								convertedDate = convertedDate.substring(0, 10).split('-');
								convertedDate = convertedDate[1] + '/' + convertedDate[2] + '/' + convertedDate[0];
	...
	function clearAndApply(data, status) {
		...
		console.log("[Updater] Updating Scope: ", this.scope);
		this.scope.model = {};
		...
		function populate(m, u, topLevel) {
			...
			if (Array.isArray(u) || (u && u.AngularList)) {
				...
				if (u.AngularList && (u.AngularList.length == 0 || u.AngularList[0].Key) && (m.length == 0 || m[0].Key)) {
					for (var k in u.AngularList) { //Foreach element in src
						if (arrayHasOwnIndex(u.AngularList, k)) {
							...
							if (loc != -1) {
								...
								//Replace all the rest as well.
								while (true) {
									try {
										loc = keysList.indexOf(u.AngularList[k]["Key"], loc + 1);
										if (loc == -1)
											break;
										debugger;
										m[loc] = populate(m[loc], u.AngularList[k], false);
					...
					if (firstWarn) {
						console.warn("[Updater] List items should have keys.");
						firstWarn = false;
					}
				}
				...
				for (var k in u) {
					if (arrayHasOwnIndex(u, k)) {
						if (m[k] !== null && typeof (m[k]) !== "undefined" && m[k].AngularList) {
							...
							if (m[k].UpdateMethod == "Remove") {
								...
								console.error("[Updater] Unknown update type:" + m[k].UpdateMethod);
							}
						}
	...
	function constructSendUpdate(useUpdateUrl, options) {
		...
		if (typeof (updateUrl) === "undefined") {
			console.warn("[Updater] Aborting. URL must be either a string or function to construct a SendUpdate method.");
			return function () {
		...
		return function (self, args) {
			...
			for (var i in args) {
				if (arrayHasOwnIndex(args, i)) {
					if (typeof (args[i]) !== "string")
						console.warn("[Updater] SendUpdate argument (" + i + ") did not resolve to a string.");
					builder += "&" + i + "=" + args[i];
				}
			}
			var url = null;
			...
			if (typeof (url) !== "string") {
				console.error("[Updater] Aborting sendUpdate. Url did not resolve to a string.");
				return;
			}
	...
	function updaterFactory($scope, hub) {
		...
		if (typeof (hub) !== "undefined") {
			if (typeof (hub.client.update) !== "undefined") {
				console.warn("[Updater] hub.client.update already defined");
			}
			hub.client.update = function (a, b) {
