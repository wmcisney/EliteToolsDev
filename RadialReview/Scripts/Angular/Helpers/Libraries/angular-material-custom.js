	...
	(function () {
		...
		function MdAutocompleteCtrl($scope, $element, $mdUtil, $mdConstant, $mdTheming, $window,
			...
			function configureWatchers() {
				...
				$scope.$watch('inputId', inputIdChange);
            ...
            function selectedItemChange(selectedItem, previousSelectedItem) {
				...
				if (!$scope.updatingSource) {
					if (selectedItem !== previousSelectedItem) {
						announceItemChange();
						console.info("autocomplete - item change " + (selectedItem || {}).Key);
					}
				} else {
					console.info("autocomplete - item change blocked " + (selectedItem || { }).Key);
                }
			...
			/*CLAY*/
			function inputIdChange(newId, oldId) {
				//if (newId !== oldId) {
				//	console.info("autocomplete - updatingSource: true");
				//	$scope.updatingSource = true;
				//	console.info("autocomplete - id changed " + newId);
				//	$timeout(function () {
				//		$scope.updatingSource = false;
				//		console.info("autocomplete - updatingSource: false");
				//	},100);
				//}
				if (newId !== oldId) {
					console.info("autocomplete - update source "+newId)
				}
			}
			...
			function handleSearchText(searchText, previousSearchText) {
				...
				//if ($scope.updatingSource) {
				//	console.info("autocomplete - handleSearchText skip");
				//	return;
				//}
				...
				getDisplayValue($scope.selectedItem).then(function (val) {
					...
					if (searchText !== val) {
						...
						if (searchText == "") {
							$scope.selectedItem = null;
							console.info("autocomplete - clearing");
						} else if (typeof (searchText) === "undefined") {
							console.info("autocomplete - searchText undefined");
							return;
						} else {
							console.info("autocomplete - not clearing: " + searchText);
						}
						/*CLAY End*/
			...
			function blur() {
				...
				if (!noBlur) {
					...
					getDisplayValue($scope.selectedItem).then(function (val) {
						/*CLAY*/
						if (val !== $scope.searchText) { //&& (val != null && $scope.searchText != "")) {
							console.info("autocomplete - resetting text");
							var valEmpty = (val == "" || val == null);
							var oldSearch = $scope.searchText;
							var searchEmpty = ($scope.searchText == "" || $scope.searchText == null);
							$scope.searchText = val;

							if (!(valEmpty && searchEmpty)) {
								try {
									setTimeout(function () {
										window.showAlert("Selection invalid. <br/>Please click a valid item from the search menu.",5000);
									}, 60);
								} catch (e) {
									console.error(e);
								}
					...
					});//$parse(attr.mdSelectedText)(scope);
				}
			}
			...
			function keydown(event) {
				switch (event.keyCode) {
					...
					case $mdConstant.KEY_CODE.ENTER:
						...
						/*CLAY*/
						console.info("autocomplete - selecting anyway");
						event.stopPropagation();
						event.preventDefault();
						select(ctrl.index);
						break;
			 ...
			 * @returns {boolean}
			 */
			function shouldHide(overrideHasSelection) {
				if (ctrl.loading && !hasMatches()) return true; // Hide while loading initial matches
				else if (overrideHasSelection && hasSelection()) {
					/*CLAY*/
					console.info("autocomplete - hiding due to selection");
			...
			function select(index) {
				...
				$mdUtil.nextTick(function () {
					getDisplayValue(ctrl.matches[index]).then(function (val) {
						...
						setLoading(false);
						/*CLAY*/
						console.info("autocomplete - force blur");
						doBlur(true);
						shouldHide(true);
					});
                }, false);
		...
		MdAutocompleteCtrl.$inject = ["$scope", "$element", "$mdUtil", "$mdConstant", "$mdTheming", "$window", "$animate", "$rootElement", "$attrs", "$q","$timeout"];
		...
		function MdAutocomplete() {
			...
			return {
				...
				scope: {
					...
					ttClearNoMatch: '@?ttMdClearNoMatch'
				},
          ...
          <p ng-repeat="message in $mdAutocompleteCtrl.messages track by $index" ng-if="message">{{message}}</p>\
            ...
            <md-input-container flex ng-if="floatingLabel">\
              ...
              <input type="search"\
                  ...
                  name="{{inputName}}"\
                  autocomplete="off"\
                  tt-clear-no-match="{{ttClearNoMatch}}"\
