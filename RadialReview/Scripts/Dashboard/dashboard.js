...
function fixTileHeight() {
	...
	if (msieversion() == false) {
		$(".tile-height-resize").each(function () {
			...
			var scrollHeight = Math.floor(Math.max(Math.max($(this).children().height(), $(this).height()), tile[0].scrollHeight));
			var tileHeight = Math.floor(tile.height()-.2);

			if (tileHeight > scrollHeight) {
				var self = $(this);
				var position = self.position();
				var a = tileHeight - position.top - Math.floor(self.outerHeight() - self.innerHeight());
