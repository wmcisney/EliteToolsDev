var Time = new function () {
	...
	this.addTimestamp = function (url) {
		...
		var res = url;
		var hasTimestamp = url.indexOf("_clientTimestamp") != -1;
		var hasTimezone = url.indexOf("_tz") != -1;
		if (!hasTimezone || !hasTimestamp) {
			res += ((url.indexOf("?") != -1) ? "&" : "?");
			arr = [];
			if (!hasTimestamp) {
				arr.push("_clientTimestamp=" + date);
			}
			if (!hasTimezone) {
				arr.push("_tz=" + (-window.tzoffset));
			}
			res += arr.join("&");
		}
		return res;
	...
	this.to12HourFormat = function (hour) {
		if (hour == 0)
			return "12 am";
		if (hour >= 12) {
			hour = hour - 12
	...
	this.localTimeZoneName = function () {
		var d = new Date();
		return d.toString().substr(35).substr(0, d.toString().substr(35).length - 1);
	}
	...
	this.createClientDatepicker = function (options) {
		...
		if (options.serverTime && options.localTime)
			throw "You cannot specify both a localTime and a serverTime";

		var selector = options.selector;
		var serverTime = options.serverTime;
		if (!serverTime && options.localTime) {
			serverTime = Time.toServerTime(options.localTime);
		}

		serverTime = this.parseJsonDate(serverTime);
		...
		var builder = '';
		builder += '<div class="input-append date ' + guid + ' ' + classes + '">';
		builder += '<input class="form-control client-date ' + classes + '" type="text" value="' + clientDateFormat(displayTime) + '" placeholder="not set" data-val="true" data-val-date="The field must be a date.">';
