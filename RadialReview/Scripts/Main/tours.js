...
function startTour(name, method) {
	...
	$.getScript("/Scripts/Tour/lib/anno.js").done(function () {
		...
		if (typeof (Tours) === "undefined") {
			Tours = {
				Fail:function(m){
					console.warn("Tours failed.",m);
					try {
						$(".anno,.anno-overlay").remove()
					} catch (e) {
						console.error(e);
					}
				},
						...
						var handler = function (e) {
							...
							if (anno._chainNext != null) {
								waitUntil(function () { return $(anno._chainNext.target).length > 0; }, function () {
									...
									console.warn("Could not load tour.");
									try{anno.hide();
									} catch (e) {
										console.error(e);
									}
				...
				appendParams: function (anno, selector, tourName, tourMethod) {
					...
					anno.onShow = function (anno, $target, $annoElem) {
						...
						tmethod = tourMethod;
						var handler = function (e) {
							if (typeof (e.target.href) !== "undefined" && e.target.href !=null && $(e.target).is(selector)) {
								e.preventDefault();
							...
							if (typeof (e.target.onclick) !== "undefined" && e.target.onclick !== null && $(e.target).is(selector)) {
								e.preventDefault();
		...
		try {
			...
			console.warn("Could not load tour.",e);
			try {
				anno.hide();
			} catch (e) {
				console.error(e);
