...
#pragma warning restore CS0618 // Type or member is obsolete
            ...
            protected void UpdateAll(Func<long, IAngularId> itemGenerater, bool forceNoSkip = false, string group = null)
            {
                var updater = rt.GetUpdater<RealTimeHub>(RealTimeHub.Keys.OrganizationId(_OrganizationId),!forceNoSkip, group);
			...
			public RTOrganizationUpdater ForceUpdate(IAngularId item,string group=null) {
				return Update(rid => item, true,group);
			}
			public RTOrganizationUpdater Update(Func<long, IAngularId> item,bool forceNoSkip = false,string group=null)
            {
                rt.AddAction(() => {
                    UpdateAll(item, forceNoSkip, group);
                });
                return this;
            }
