...
#pragma warning disable CS0618 // Type or member is obsolete
		private async Task TestPlan(UserModel userModel, PaymentPlanType plan, int baseCharge,
			...
			using (var mock = Scheduler.Mock()) {
				...
				{
					var taskResult = await TaskAccessor.EnqueueTask_Test(res.organization.PaymentPlan.Task, now);
					...
					var taskResult = await TaskAccessor.EnqueueTask_Test(nextTask, now.AddDays(28));
					var result = mock.Perform<HangfireChargeResult>((string)taskResult.Response);
					Assert.AreEqual(0, result.PaymentResult.amount_settled);
					nextTask = taskResult.NewTasks.Single();
				}{
					var taskResult = await TaskAccessor.EnqueueTask_Test(nextTask, now.AddDays(31));
					var result = mock.Perform<HangfireChargeResult>((string)taskResult.Response);
					...
					var taskResult = await TaskAccessor.EnqueueTask_Test(nextTask, now.AddDays(62));
					var result = mock.Perform<HangfireChargeResult>((string)taskResult.Response);
					Assert.AreEqual(baseCharge, result.PaymentResult.amount_settled);
					DbCommit(s => {
						for (var i = 0; i < 9; i++) {
					...
					var taskResult = await TaskAccessor.EnqueueTask_Test(nextTask, now.AddDays(74));
					var result = mock.Perform<HangfireChargeResult>((string)taskResult.Response);
					Assert.AreEqual(chargeAnd19Users_L10, result.PaymentResult.amount_settled);
					nextTask = taskResult.NewTasks.Single();
				}{
					var taskResult = await TaskAccessor.EnqueueTask_Test(nextTask, now.AddDays(93));
					var result = mock.Perform<HangfireChargeResult>((string)taskResult.Response);
					...
					var taskResult = await TaskAccessor.EnqueueTask_Test(nextTask, now.AddDays(73));
					var result = mock.Perform<HangfireChargeResult>((string)taskResult.Response);
					Assert.AreEqual(chargeAnd19Users_L10, result.PaymentResult.amount_settled);
					nextTask = taskResult.NewTasks.Single();
				}{
					var taskResult = await TaskAccessor.EnqueueTask_Test(nextTask, now.AddDays(94));
					var result = mock.Perform<HangfireChargeResult>((string)taskResult.Response);
